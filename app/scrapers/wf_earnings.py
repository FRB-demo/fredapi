"""Wells Fargo Quarterly Earnings scraper."""
...
class WFEarningsScraper(BaseScraper):
    """Scraper for Wells Fargo quarterly earnings documents."""

    name = "wf_earnings"
    display_name = "Wells Fargo Quarterly Earnings"
    base_url = "https://www.wellsfargo.com/about/investor-relations/quarterly-earnings/"
    doc_type = "pdf"

    def scrape(self) -> List[ScrapedDocument]:
        """Scrape Wells Fargo quarterly earnings page for PDF links."""
        ...
        try:
            ...
            self.last_error = f"Failed to fetch earnings page: {e}"
            return documents
        ...
        for url, title in pdf_links:
            ...
            doc = ScrapedDocument(
                ...
                title=title,
                date=self._extract_quarter_date(title),
    ...
    def _extract_quarter_date(self, title: str) -> str | None:
        """Try to extract quarter/year from title text."""
        match = re.search(r"(Q[1-4]\s*\d{4})", title, re.IGNORECASE)
        if match:
            return match.group(1)

        match = re.search(
            r"((?:First|Second|Third|Fourth)\s+Quarter\s+\d{4})", title, re.IGNORECASE
        )
        if match:
            return match.group(1)
